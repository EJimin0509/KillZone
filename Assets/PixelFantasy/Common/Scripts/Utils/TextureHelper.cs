...
namespace Assets.PixelFantasy.Common.Scripts.Utils
{
    ...
    public static class TextureHelper
    {
        ...
        public static Color32[] Repaint(Color32[] pixels, int width, int height, Color32 paint, List<Color32> palette)
        {
            var dict = new Dictionary<Color32, int>();

            for (var x = 0; x < width; x++) // TODO: Hardcoded values.
            {
                for (var y = 0; y < height - 64; y++)
                {
            ...
            const int max = 4;
            var colors = dict.Count > max ? dict.OrderByDescending(i => i.Value).Take(max).Select(i => i.Key).ToList() : dict.Keys.ToList();
            ...
            if (colors.Count < 2 || colors.Count > max)
            {
                throw new NotSupportedException($"Sprite should have 2-{max} colors only (+black outline).");
            }
