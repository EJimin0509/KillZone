...
namespace Assets.PixelFantasy.PixelHeroes4D.Common.Scripts.CharacterScripts
{
    public class CharacterBuilder : CharacterBuilderBase
    {
        ...
        public override void Rebuild(bool forceMerge = false)
        {
            ...
            {
                { "F_", new[] { "Cape", "Back", "Body", "Armor", "Arms", "Bracers", "Head", "Horns", "Eyes", "Mask", "Hair", "Ears", "Helmet", "Shield", "Weapon" } },
                { "S_", new[] { "Cape", "Back", "Shield", "Body", "Armor", "Arms", "Bracers", "Head", "Horns", "Eyes", "Mask", "Hair", "Ears", "Helmet", "Weapon" } },
                { "BlockS_", new[] { "Shield" } },
                { "SlashS_2", new[] { "Arms", "Bracers" } },
                { "B_", new[] { "Weapon", "Shield", "Body", "Armor", "Arms", "Bracers", "Cape", "Back", "Head", "Horns", "Hair", "Ears", "Helmet" } },
                { "DeathB_", new[] { "Cape", "Back", "Body", "Armor", "Arms", "Bracers", "Head", "Horns", "Eyes", "Mask", "Hair", "Ears", "Helmet", "Shield", "Weapon" } }
            };
        ...
        private static Color32[] Split(Color32[] pixels, params string[] filter)
        {
            var overlay = new Color32[pixels.Length];
            var frames = new List<string>();

            foreach (var frame in Layout.Keys)
            {
                foreach (var f in filter)
                {
                    if (f[0] == '!' && !frame.Contains(f.Substring(1)))
                    {
                        frames.Add(frame);
                    }
                    else if (frame.Contains(f))
                    {
                        frames.Add(frame);
                    }
                }
            ...
            {
                var xMin = Layout[frame][0];
                var xMax = Layout[frame][0] + Layout[frame][2];
                var yMin = Layout[frame][1];
                var yMax = Layout[frame][1] + Layout[frame][3];
